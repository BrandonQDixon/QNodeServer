...
describe('basic server test with test double server', () => {
    ...
    it('should get and post to different endpoints with the same path', async (done) => {
        ...
        const getResponse: IQNodeResponse = await testDoublePlugin.testEndpoint(TEST_ENDPOINTS.numberGet.verb, TEST_ENDPOINTS.numberGet.path, getRequest);
        expect(getResponse.body.number).toBe(getBody.number);

        //post request, set number
        const postResponse: IQNodeResponse = await testDoublePlugin.testEndpoint(TEST_ENDPOINTS.numberPost.verb, TEST_ENDPOINTS.numberPost.path, postRequest);
    ...
    it('should get, post, and get again to same endpoint path', async (done) => {
        ...
        //first get request, default (0) + input
        const getOneResponse: IQNodeResponse = await testDoublePlugin.testEndpoint(TEST_ENDPOINTS.numberGet.verb, TEST_ENDPOINTS.numberGet.path, getRequest);
        ...
        const getTwoResponse: IQNodeResponse = await testDoublePlugin.testEndpoint(TEST_ENDPOINTS.numberGet.verb, TEST_ENDPOINTS.numberGet.path, getRequest);
