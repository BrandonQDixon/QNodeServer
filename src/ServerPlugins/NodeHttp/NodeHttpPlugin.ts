...
export class NodeHttpPlugin implements IQServerPlugin {
    ...
    private getRequestHandler(): RequestListener {
        return async (rawRequest: any, rawResponse: any) => {
            ...
            if (!foundEndpoint) {
                throw new Error(`Error in NodeHttpPlugin: endpoint callback was falsey for: ${foundEndpoint.metadata.verb} ==> ${foundEndpoint.metadata.path}`);
            }
            const result: IQNodeResponse = await foundEndpoint.callback(rawRequest);
    ...
    private getRequestEndpoint(request: any): IQNodeConcreteEndpoint {
        return this.endpointRoutes.find(route => {
            ...
            return request.url.indexOf(path) === 0 && request.verb === route.metadata.verb;
        });
    }
